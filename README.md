...
**VSP-channel transactions are excluded from this dashboard by standing request.** Of 5,575 valid transactions in the DAILY TRANSACTIONS tab, 99 tagged with the VSP channel were removed, leaving **5,476 transactions** that power every number on this page.

## Data coverage

5,476 real transactions from Jan 1, 2021 to Sep 25, 2026, each with complete financials (revenue, cost, commission, profit), plus 5,862 expense records from Dec 29, 2020 to Sep 25, 2026. Every KPI, chart, and table is computed live from that embedded dataset — nothing is padded, sampled, or estimated. Destination and expense-type blanks/placeholders (e.g. destination "NA" for B2B packages) are mapped to "Unspecified"; free-text variants of the same place (e.g. "CAGAYAN" vs "CAGAYAN DE ORO" vs "CDO") are not merged.
