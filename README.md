...
**VSP-channel transactions are excluded from this dashboard by standing request.** Of 5,500 valid transactions in the DAILY TRANSACTIONS tab, 99 tagged with the VSP channel were removed, leaving **5,401 transactions** that power every number on this page.

## Data coverage

5,401 real transactions from Jan 1, 2021 to Sep 7, 2026, each with complete financials (revenue, cost, commission, profit), plus 5,738 expense records from Dec 29, 2020 to Sep 7, 2026. Every KPI, chart, and table is computed live from that embedded dataset — nothing is padded, sampled, or estimated. Destination and expense-type blanks/placeholders (e.g. destination "NA" for B2B packages) are mapped to "Unspecified"; free-text variants of the same place (e.g. "CAGAYAN" vs "CAGAYAN DE ORO" vs "CDO") are not merged.
