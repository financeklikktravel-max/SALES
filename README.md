...
- **Top KPIs** — total sales, revenue, orders, AOV, gross profit, profit margin, cash flow, total expenses, and **net cash flow** (cash flow − expenses), scoped to the *latest* period matching the granularity control (click Daily and see today's numbers, Weekly for this week, Monthly for the full calendar month, etc.). Cash flow specifically only counts **completed** transactions, recognized on their **processed date** — every other KPI uses the booking date and isn't status-filtered.
