...
- **Breakdown by period** — total SRP, cash flow, revenue (cash flow − that period's expenses), and transactions for every period at the selected granularity (most recent first); monthly periods are labeled with their real calendar range (e.g. "Jul 1–31"). SRP and transaction counts are by booking date; cash flow is completed-only, by processed date — so a period can show SRP with little or no cash flow if those bookings haven't been processed yet. The latest row's revenue always matches the Top KPI "Total Revenue" card.
...
**VSP-channel transactions are excluded from this dashboard by standing request.** Of 5,523 valid transactions in the DAILY TRANSACTIONS tab, 99 tagged with the VSP channel were removed, leaving **5,424 transactions** that power every number on this page.

## Data coverage

5,424 real transactions from Jan 1, 2021 to Sep 12, 2026, each with complete financials (revenue, cost, commission, profit), plus 5,738 expense records from Dec 29, 2020 to Sep 12, 2026. Every KPI, chart, and table is computed live from that embedded dataset — nothing is padded, sampled, or estimated. Destination and expense-type blanks/placeholders (e.g. destination "NA" for B2B packages) are mapped to "Unspecified"; free-text variants of the same place (e.g. "CAGAYAN" vs "CAGAYAN DE ORO" vs "CDO") are not merged.
